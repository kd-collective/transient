         ...
         (let ((,depth (1+ (minibuffer-depth))) ,setup ,exit)
           ...
           (unwind-protect
               (progn
                 ...
                 (advice-add 'abort-recursive-edit :after ,exit)
                 ,@body)
             (remove-hook 'minibuffer-setup-hook ,setup)
             (remove-hook 'minibuffer-exit-hook ,exit)
             (advice-remove 'abort-recursive-edit ,exit)))
       ,@body)))
  ...
  (let ((depth (minibuffer-depth))
    ...
    (setq post-command
          (lambda ()
            ...
            (when (and (eq this-command command)
    ...
    (setq abort-minibuffer
          (lambda ()
            (transient--debug
             'post-command
             (lambda () (format ", mini: %s|%s"
                           (minibuffer-depth)
                           depth)))
            (when (and (or (memq this-command transient--abort-commands)
    ...
    (let ((inhibit-message (not (eq transient--debug 'message))))
      (if (symbolp arg)
          (message "-- %-16s (cmd: %s, event: %S, exit: %s%s)"
                   ...
                   transient--exitp
                   (if args (apply (car args) (cdr args)) ""))
